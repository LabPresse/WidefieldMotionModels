function String  = getLaTeX_(Number,varargin) %#ok<DEFNU>
%% Interpret Input:
  if isstring(Number) | iscellstr(Number) | ischar(Number)
      String = split(Number, [","    " "]);
      Number = eval(String +  ";")        ;
  end
%% Operate Over Each <Number>:
  Numbers  = Number;
  ...
  % PROPOSED Measure:   Number*ProposedUnit^UnitExponent
      ...
      if UnitExponent == 1 | Unit == ProposedUnit    %#ok<*OR2>  (| not ||) for multiple {Number}s.
          ...
          if     Coefficient(~mod(Coefficient,1)) == Coefficient(:);    Digits=1;    digCoeff = 0;    digitType = 'd';
          elseif 10*Coefficient(~mod(10*Coefficient,1)) == 10*Coefficient(:);        digCoeff = 0;    digitType = 'f';
  ...
  %{%
          ...
          if   ~exist("Unit",'var')
               if         Exponent    == -1;   %String = ""+num2str(Coefficient/10, '%#3.'+string(Digits)  + digitType) + "";
                   ...
                   elseif Coefficient == -1;  String(each) =                                                                  "-10^{"+string(Exponent)+"}";   
...
%{
  % Eliminate "." when rounding to whole number.
%         if~Digits;    STRING       = split(String(each), ".");
          if~Digits;    STRING       = split(String{each}, ".")%;
                        String(each) =  join(STRING      , "" );
          end
  %}
      else
        %%%%%     %%%%%     %%%%%
